    ...
    */
    ...
    public function setErrorCode(string $error_code): static
    {
        ...
        if ((mb_strlen($error_code) > 3)) {
            throw new InvalidArgumentException('invalid length for $error_code when calling RefundResponse., must be smaller than or equal to 3.');
        }
        if ((mb_strlen($error_code) < 1)) {
            throw new InvalidArgumentException('invalid length for $error_code when calling RefundResponse., must be bigger than or equal to 1.');
        }
        if ((!preg_match("/^\\d{1,3}$/", ObjectSerializer::toString($error_code)))) {
            throw new InvalidArgumentException("invalid value for \$error_code when calling RefundResponse., must conform to the pattern /^\\d{1,3}$/.");
        }
    ...
    public function setErrorMessage(?string $error_message): static
    {
        ...
        if ((mb_strlen($error_message) > 512)) {
            throw new InvalidArgumentException('invalid length for $error_message when calling RefundResponse., must be smaller than or equal to 512.');
        }
        if ((mb_strlen($error_message) < 0)) {
            throw new InvalidArgumentException('invalid length for $error_message when calling RefundResponse., must be bigger than or equal to 0.');
        }
        if ((!preg_match("/^[ -~А-Яа-яЁё№]*$/", ObjectSerializer::toString($error_message)))) {
            throw new InvalidArgumentException("invalid value for \$error_message when calling RefundResponse., must conform to the pattern /^[ -~А-Яа-яЁё№]*$/.");
        }
