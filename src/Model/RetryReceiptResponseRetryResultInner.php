<?php
/**
 * RetryReceiptResponseRetryResultInner
 *
 * PHP version 8.1
 *
 * @package  SberPay
...
/**
 * RetryReceiptResponseRetryResultInner Class Doc Comment
 *
 * @package  SberPay
 * @author   OpenAPI Generator team
 * @link     https://openapi-generator.tech
 ...
 */
class RetryReceiptResponseRetryResultInner implements ModelInterface, ArrayAccess, JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static string $openAPIModelName = 'retryReceiptResponse_retryResult_inner';
    ...
    */
    ...
    public function listInvalidProperties(): array
    {
        ...
        if (!is_null($this->container['result']) && (mb_strlen($this->container['result']) > 128)) {
            $invalidProperties[] = "invalid value for 'result', the character length must be smaller than or equal to 128.";
        }

        if (!is_null($this->container['result']) && !preg_match("/^[ -~]*$/", $this->container['result'])) {
            $invalidProperties[] = "invalid value for 'result', must be conform to the pattern /^[ -~]*$/.";
        }
    ...
    public function setReceiptId(?string $receipt_id): static
    {
        ...
        if ((mb_strlen($receipt_id) > 36)) {
            throw new InvalidArgumentException('invalid length for $receipt_id when calling RetryReceiptResponseRetryResultInner., must be smaller than or equal to 36.');
        }
        if ((mb_strlen($receipt_id) < 36)) {
            throw new InvalidArgumentException('invalid length for $receipt_id when calling RetryReceiptResponseRetryResultInner., must be bigger than or equal to 36.');
        }
        if ((!preg_match("/^[a-f0-9\\-]+$/", ObjectSerializer::toString($receipt_id)))) {
            throw new InvalidArgumentException("invalid value for \$receipt_id when calling RetryReceiptResponseRetryResultInner., must conform to the pattern /^[a-f0-9\\-]+$/.");
        }
     ...
     * Sets result
     *
     * @param string|null $result Результат обработки чека: * `Успешно` * `Чек не найден` * `Чек уже изменен ранее` * `Чек уже зарегистрирован ранее` * `Ожидается статус регистрации в кассовом сервисе. Чек не может быть отправлен.`
     *
     * @return $this
     */
    public function setResult(?string $result): static
    {
        ...
        if ((mb_strlen($result) > 128)) {
            throw new InvalidArgumentException('invalid length for $result when calling RetryReceiptResponseRetryResultInner., must be smaller than or equal to 128.');
        }
        if ((!preg_match("/^[ -~]*$/", ObjectSerializer::toString($result)))) {
            throw new InvalidArgumentException("invalid value for \$result when calling RetryReceiptResponseRetryResultInner., must conform to the pattern /^[ -~]*$/.");
        }
